...
function root({
  ...
  return {
    title: shop?.meta_title.value,
    titleTemplate: '',
    description: truncate(shop?.meta_description.value ?? ''),
    handle: '@shopify',
    url,
    robots: {
      noIndex: false,
...
function product({
  ...
  return {
    ...
    jsonLd: productJsonLd({product, selectedVariant, url, productReviews}),
  };
}
